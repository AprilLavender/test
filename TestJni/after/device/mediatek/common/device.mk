...
# meta tool
PRODUCT_PROPERTY_OVERRIDES += ro.mediatek.version.branch=L1.MP6
...
# ROCO_ShiYouHui add for custom override Customer configurations  @2015-10-08 >>>
ifneq ($(strip $(ROCO_PROJECT)),)
	ifneq ($(wildcard $(MTK_PROJECT_FOLDER)/roco/$(ROCO_PROJECT)/custom.conf),)
		PRODUCT_COPY_FILES += $(MTK_PROJECT_FOLDER)/roco/$(ROCO_PROJECT)/custom.conf:system/etc/custom.conf
	else
...
# ROCO_ShiYouHui add for custom override GMS interface  @2015-10-08 >>>
ifneq ($(strip $(ROCO_PROJECT)),)
	ROCO_CUSTOM_GMS := $(MTK_PROJECT_FOLDER)/roco/$(ROCO_PROJECT)/gms.mk
endif
# GMS interface
ifdef BUILD_GMS
ifeq ($(strip $(BUILD_GMS)), yes)
ifneq ($(strip $(ROCO_PROJECT)),)
	ifeq ($(ROCO_CUSTOM_GMS), $(wildcard $(ROCO_CUSTOM_GMS)))
$(call inherit-product-if-exists, $(ROCO_CUSTOM_GMS))
	else
...
# ROCO_ShiYouHui add for product_roco.mk  @2015-10-08 >>>
ifneq ($(strip $(ROCO_PROJECT)),)
$(call inherit-product-if-exists, $(MTK_PROJECT_FOLDER)/roco/$(ROCO_PROJECT)/product_roco.mk)
endif
# ROCO_ShiYouHui add for product_roco.mk  @2015-10-08 <<<
PRODUCT_PACKAGES += FactoryMode
PRODUCT_PROPERTY_OVERRIDES += ro.custom.build.version=ROCO-2016-08-06
PRODUCT_PACKAGES += GestureAnimation
PRODUCT_PACKAGES += GsensorCalibration
PRODUCT_PACKAGES += AutoGen
DEVICE_PACKAGE_OVERLAYS += $(MTK_PROJECT_FOLDER)/roco/$(ROCO_PROJECT)/overlay
ifeq ($(strip $(ROCO_AUTOGEN_ADDR)), yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.roco_autogen_addr=1
else
  PRODUCT_PROPERTY_OVERRIDES += ro.roco_autogen_addr=0	
endif

PRODUCT_PACKAGES += Freg